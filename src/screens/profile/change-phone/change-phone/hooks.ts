...
const defaultValues: PhoneNumberForm = {
  phone: '',
};
...
export function usePhoneNumberForm(): UseLoginFormReturn {
  ...
  const handleSubmit = useCallback(async (values: PhoneNumberForm) => {
    Keyboard.dismiss();
    try {
      setIsLoading(true);
      auth()
        ...
        .on(
          ...
          phoneAuthSnapshot => {
            ...
            if (phoneAuthSnapshot.state === auth.PhoneAuthState.CODE_SENT) {
              navigation.navigate(UserStackRoutes.VerifyUpdatedNumber, {
